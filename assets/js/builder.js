...
${end.comment}` : end.comment;
  ...
  function packageValidator(schema2) {
    ...
    return (value) => {
      if (!validate2(value)) {
        throw new Error(validate2.errors.map((error) => `${error.instancePath || "/"}: ${error.message}${error.params.additionalProperty ? ` (${error.params.additionalProperty})` : ""}`).join("\n"));
      }
  ...
  function failure(error, attempt) {
    const data = error instanceof ApiError ? error.data : void 0;
  ...
  function setIdentity(element, ref2) {
    ...
    const attempt = (deadline) => {
      ...
      try {
        ...
        if (!warned) {
          warned = true;
          console.error("[AllTerrain Forms] The shell refused a window identity.", error, ref2);
        }
